...
class RewardCalculationRequest(BaseModel):
    epoch: int | None = Field(
        default=None,
        ge=0,
        description="Reward calculation epoch. If omitted, the default epoch is resolved automatically.",
        examples=[DEMO_EPOCH],
    )
...
class RewardRead(BaseModel):
    ...
    active_stake_lamports: int = Field(description="Active stake in lamports.", examples=[14887656841])
    ...
    gross_mev_reward_lamports: int = Field(description="Calculated MEV reward in lamports.", examples=[500000000])
    gross_block_reward_lamports: int = Field(
        description="Calculated block reward in lamports.",
        examples=[500000000],
    )
    gross_reward_lamports: int = Field(description="Gross reward in lamports.", examples=[1000000000])
    payable_reward_lamports: int = Field(description="Payable reward in lamports.", examples=[1000000000])
...
class StakerStatsRead(BaseModel):
    ...
    gross_total_lamports: int = Field(description="Gross total rewards in lamports.", examples=[1000000000])
    payable_total_lamports: int = Field(description="Payable total rewards in lamports.", examples=[1000000000])
